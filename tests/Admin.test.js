...
describe("Admin Tests", () => {
  ...
  test("Only Admin Can Get Users List", async (done) => {
    ...
    expect(getAllUsersRes.body.USERS[0].email).toBe(userAdminMock.email);
    expect(getAllUsersRes.body.USERS.length > 0).toBe(true);
  ...
  test("Passwords must be stored with hash", async (done) => {
    ...
    expect(infoRes.status).toBe(200);
    const userMock = infoRes.body.USERS.filter(
      (user) => user.email === userRegisterMock.email
    )[0];
